...
manifest.json # PWA manifest(圖示、名稱、standalone 顯示模式)
sw.js         # Service worker:快取 app shell,離線可開;不攔截跨網域/非 GET 請求(Gemini API 呼叫永遠直接走網路)
icons/        # PWA 圖示(icon-192/512、maskable、apple-touch-icon、favicon),由 hud-core 造型產生,腳本見開發紀錄
```
